...
import (
	...
	_ "github.com/anirudhraja/protolite/conformance_test/generated/google/protobuf"
)

type Harness struct{
	pl protolite.Protolite
	// diagnostics
	skippedProto2OrEditions int
	skippedJSPB             int
	skippedTextFormat       int
	skippedTextPayload      int
	parseErrors             int
	serializeErrors         int
	runtimeErrors           int
	totalRequests           int
...
func main() {
	// Configure wire behavior: hard-coded for conformance runs (no env flags)
	wire.SetConfig(wire.Config{
		FillMissingScalarDefaultsOnDecode: false,
	})
	...
	log.Printf("conformance-go: using PROTOLITE_PROTOS_DIR=%s", protosRoot)
	...
	totalSkipped := h.skippedProto2OrEditions + h.skippedJSPB + h.skippedTextFormat + h.skippedTextPayload
	log.Printf("Conformance run complete | processed=%d | skipped_total=%d (proto2/editions=%d, jspb=%d, text_payload=%d, text_format_out=%d) | errors: parse=%d, serialize=%d, runtime=%d",
		totalRuns, totalSkipped, h.skippedProto2OrEditions, h.skippedJSPB, h.skippedTextPayload, h.skippedTextFormat, h.parseErrors, h.serializeErrors, h.runtimeErrors)
}
...
func (h *Harness) RunTest(req *conformancepb.ConformanceRequest) *conformancepb.ConformanceResponse {
	...
	// Only count errors for scored tests (exclude text-format suite/probes)
	countErrors := true
	if req.GetTestCategory() == conformancepb.TestCategory_TEXT_FORMAT_TEST {
		countErrors = false
	}
	// If the requested output is text format, we don't support it at all.
	// Skip immediately without attempting to parse any payload to avoid spurious parse errors.
	if req.GetRequestedOutputFormat() == conformancepb.WireFormat_TEXT_FORMAT {
		h.skippedTextFormat++
	...
	case *conformancepb.ConformanceRequest_JsonPayload:
		// Use Google's protojson to validate and parse JSON correctly
		mt, err := protoregistry.GlobalTypes.FindMessageByName(protoreflect.FullName(req.MessageType))
		if err != nil {
			resp.Result = &conformancepb.ConformanceResponse_RuntimeError{
				RuntimeError: fmt.Sprintf("unknown message type %q: %v", req.MessageType, err),
			}
			return resp
		}
		pm := mt.New().Interface()
		...
		if uerr := opts.Unmarshal([]byte(payload.JsonPayload), pm); uerr != nil {
			if countErrors {
				h.parseErrors++
			}
			resp.Result = &conformancepb.ConformanceResponse_ParseError{
				ParseError: fmt.Sprintf("parse error: %v", uerr),
			}
			return resp
		}
		// Marshal with Google's library to get validated protobuf bytes
		bytesOut, merr := proto.Marshal(pm)
		if merr != nil {
			if countErrors {
				h.serializeErrors++
			}
			resp.Result = &conformancepb.ConformanceResponse_SerializeError{
				SerializeError: fmt.Sprintf("serialize error: %v", merr),
			}
			return resp
		}
		// Decode with protolite and re-encode to test MarshalWithSchema on round-trip
		decoded, derr := h.pl.UnmarshalWithSchema(bytesOut, req.MessageType)
		if derr != nil {
			if countErrors {
				h.runtimeErrors++
			}
			resp.Result = &conformancepb.ConformanceResponse_RuntimeError{
				RuntimeError: fmt.Sprintf("decode error: %v", derr),
			}
			return resp
		}
		obj = decoded
