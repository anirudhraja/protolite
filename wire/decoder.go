...
func (d *Decoder) DecodeTypedField(field *schema.Field, wireType WireType) (interface{}, bool, error) {
	...
	case schema.KindPrimitive:
		...
		// A bytes field annotated with (gogoproto.customtype) carries a
		// JSON-encoded value on the wire; decode it back into a Go value.
		if field.CustomType && !isPacked {
			decoded, err := decodeCustomType(value)
			if err != nil {
...
// decodeCustomType interprets the raw bytes of a (gogoproto.customtype) field
// as a JSON document and decodes it into a Go value. Numbers are preserved as
// json.Number to match the rest of the library and avoid precision loss.
func decodeCustomType(value interface{}) (interface{}, error) {
	raw, ok := value.([]byte)
	if !ok {
		return nil, fmt.Errorf("(gogoproto.customtype) field expected bytes, got %T", value)
	}
	if len(raw) == 0 {
		return nil, nil
	}
	...
	if err := dec.Decode(&out); err != nil {
		return nil, fmt.Errorf("unmarshal (gogoproto.customtype) json: %w", err)
	}
	return out, nil
}
